        ...
        <ul class="flex flex-wrap flex-row justify-end list-reset m-0">
            <li class="relative flex items-center">
                <div class="flex justify-between ml-auto text-slate-500 -my-1 flex items-center justify-center hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-300 py-1.5 ring-1 ring-slate-900/10 rounded-md pl-2 pr-2 hover:ring-slate-300 dark:bg-slate-800 dark:highlight-white/5 dark:hover:bg-slate-700">
                    ...
                    <svg id="search" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            ...
            <li class="relative flex items-center">
                <div class="ml-2 -my-1 relative">
                    ...
                    <ul id="user-info" class="hidden absolute z-40 top-full right-0 bg-white rounded-lg ring-1 ring-slate-900/10 shadow-lg overflow-hidden w-36 py-1 text-sm text-slate-700
                                font-semibold dark:bg-slate-800 dark:ring-0 dark:highlight-white/5 dark:text-slate-300 mt-4">
                        <li class="{{$global_user['uid'] > 0 ? '' : 'hidden'}} py-1 px-2 flex items-center cursor-pointer pt-5 flex items-center cursor-pointer justify-center">
                            <img class="w-6 h-6 rounded-full bg-white mr-1" src="{{$global_user['avatar']}}" alt="">
                            {{$global_user['nickname']}}
                        </li>
                        <li id="user-info-update" class="{{$global_user['uid'] > 0 ? '' : 'hidden'}} py-1 px-2 flex items-center cursor-pointer justify-center pt-5">
                            修改资料
                        </li>
            ...
            <li>
                <a class="inline-block px-2 no-underline text-grey-darkest hover:text-grey-darker font-bold text-lg">
                    <div class="relative flex items-center border-slate-200 dark:border-slate-800">
                        <button type="button" id="change-theme" aria-haspopup="true"
                            ...
                            <span class="hidden dark:inline">
                                <svg viewBox="0 0 24 24" fill="none" class="w-8 h-8">
                                    <path class="fill-transparent" fill-rule="evenodd" clip-rule="evenodd" d="M17.715 15.15A6.5 6.5 0 0 1 9 6.035C6.106 6.922 4 9.645 4 12.867c0 3.94 3.153 7.136 7.042 7.136 3.101 0 5.734-2.032 6.673-4.853Z">
                                    </path>
                                    ...
                                    <path class="fill-slate-400 dark:fill-slate-500" fill-rule="evenodd" clip-rule="evenodd" d="M17 3a1 1 0 0 1 1 1 2 2 0 0 0 2 2 1 1 0 1 1 0 2 2 2 0 0 0-2 2 1 1 0 1 1-2 0 2 2 0 0 0-2-2 1 1 0 1 1 0-2 2 2 0 0 0 2-2 1 1 0 0 1 1-1Z">
                                    </path>
                                </svg>
                            </span>
                        </button>
    ...
    <div class="relative h-screen bg-white w-60 max-w-[calc(100%-3rem)] p-6 dark:bg-slate-800">
        ...
        <nav class="relative">
            <ul class="list-reset m-0 w-full md:w-auto">
                ...
                <li>
                    <a href="/"
                       class="inline-block px-4 py-3 no-underline text-grey-darkest hover:text-grey-darker font-bold text-lg {{$select_nav==='home' ? 'text-indigo-600' : ''}}">Home</a>
                </li>
                @foreach ($global_cate as $cate)
                    <li>
                        <a href="{{route('PtBlog.categories', ['id' => $cate['id']])}}"
                           class="inline-block px-4 py-3 no-underline text-grey-darkest hover:text-grey-darker font-bold text-lg {{$select_nav==='categories_'.$cate['id'] ? 'text-indigo-600' : ''}}">{{$cate['name']}}</a>
                    </li>
                @endforeach
                @foreach ($global_nav as $nav)
                    <li>
                        <a href="{{$nav['url']}}"
                           class="inline-block px-4 py-3 no-underline text-grey-darkest hover:text-grey-darker font-bold text-lg {{ request()->path() === $nav['url']  ? 'text-indigo-600' : ''}}">{{$nav['name']}}</a>
                    </li>
                @endforeach
            </ul>
        </nav>
