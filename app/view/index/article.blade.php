...
@section('content')
    <div class="flex justify-center grow mx-auto lg:max-w-6xl px-0 w-full mt-6 lg:px-10">
        <div class="mb-5 flex flex-col justify-between p-4 rounded-md border-solid border-zinc-100 dark:border-slate-800 mx-5 w-full lg:border-2 lg:mx-auto lg:w-[50rem]">
            <div>
                <h1 class="text-2xl text-slate-900 font-normal mb-2 text-center dark:text-slate-200">
                    {{$data['title']}}
                </h1>
                ...
                @endif
                <div class="prose max-w-full mx-auto nuxt-content break-words overflow-hidden">
            ...
            <div class="w-full mx-auto bg-white rounded-lg bg-white dark:bg-slate-900 p-1 mt-5">
                    ...
                    <div class="comment">
                        <div class="flex">
                            <img src="{{$comment['user']['avatar'] ?: '/app/pt_blog/image/default.png'}}"
                                 alt="{{$comment['user']['nickname']}}" class="w-10 h-10 mr-2 rounded-md"/>
                            <div>
                    ...
                    <div class="flex justify-center mt-4 text-gray-500 dark:text-blue-300 font-medium">
                        <nav class="flex items-center" aria-label="Pagination">
                            @foreach ($comments['links'] as $link)
                                ...
                                @else
                                    <a href="{{$link['url']}}"
                                       class="px-3 py-2 font-bold {{$link['active'] ? 'text-2xl text-indigo-600 dark:text-blue-300' : 'text-base'}}">
                                        {{$link['label']}}
                                    </a>
                                @endif
                            @endforeach
                ...
                @endif
                <form action="" class="w-full grid">
                    <label for="comment-textarea" class="text-3xl my-6">评论</label>
                    <textarea name="comment" id="comment-textarea" data-id="{{$data['id']}}" data-pid="0" data-nickname="" placeholder="请输入你的评论"
                              class="outline-none bg-gray-50 p-2 rounded flex-grow border-0 dark:bg-slate-800 dark:highlight-white dark:hover:bg-slate-700"></textarea>
                    <div class="py-4">
